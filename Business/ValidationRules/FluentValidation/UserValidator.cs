...
{
    public class UserValidator:AbstractValidator<SignIn>
    {
        public UserValidator()
        {
            RuleFor(x => x.username).NotEmpty().WithMessage("Kullanıcı Adı Boş Geçilemez");
            RuleFor(x => x.username).NotEmpty().WithMessage("Kullanıcı Soyadı Boş Geçilemez");
            //RuleFor(x => x.).NotEmpty().WithMessage("Telefon Numarası Boş Geçilemez");
